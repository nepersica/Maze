...
//------------------------------------------------------------------------------
	...
	struct CheckButton : Widget {
		CheckButton(Point xy, int w, int h, const string& label, Callback cb)
			: Widget(xy, w, h, label, cb)
		{
		...
		void SetValue()
		{
			/*Fl_Check_Button* p = (Fl_Check_Button *)pw;
			p->set();*/
			dynamic_cast<Fl_Check_Button *>(pw)->set();
			//Fl_Check_Button *p = dynamic_cast<Fl_Check_Button *>(pw);
			//p->set();
		}
